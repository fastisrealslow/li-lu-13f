...
async function renderHomework() {
  ...
  const rows = candidates.map((c,i) => {
    ...
    const invBadges = c.investors.map(inv => {
      ...
      return `<span onclick="switchInvestor('${inv.id}');switchTab('current');" style="cursor:pointer;display:inline-flex;align-items:center;gap:3px;padding:3px 9px;background:rgba(212,168,83,0.18);border:1px solid rgba(212,168,83,0.55);border-radius:10px;font-size:.62rem;color:#f5d98a;font-weight:600;white-space:nowrap;" title="${isEn2?'Position':'仓位'}: ${wLabel}">${inv.name}${chgBadge} <span style="color:#fbbf24;font-size:.6rem;font-weight:700;">${wLabel}</span></span>`;
    }).join(' ');
