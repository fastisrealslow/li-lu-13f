...
function spinDateLabel(key) { return key === 'recordDate' ? spinLabel('记录日','Record date') : spinLabel('分派日','Distribution date'); }
...
function spinPrices(e) {
  ...
  return `<section class="sd-section"><h4>${spinLabel('价格参考','Price reference')}</h4><p class="sd-muted">${spinLabel('价格变动不是母子公司合计收益；历史行情配对尚需核对，不用于判断事件完成或强制卖压。','Price moves are not combined parent/child returns. Legacy price mappings need verification and do not establish completion or forced selling.')}</p>
    ${typeof parent.changePct === 'number' ? `<p>${spinLabel('母公司参考区间变动','Parent reference-period move')}: <b>${spinPct(parent.changePct)}</b> · ${spinEscape(parent.firstAnnouncementDate || parent.startDate || '—')} → ${spinEscape(parent.updatedAt || '—')}</p>` : ''}
...
function spinCard(e, pref={}) {
  ...
  const type = typeof e.type === 'object' ? e.type[lang === 'en' ? 'label_en' : 'label_zh'] : e.type;
  return `<article class="sd-card ${pref.read === e.fingerprint ? 'sd-read' : ''}" data-event="${spinEscape(e.id)}">
    <div class="sd-card-top"><span class="sd-type">${spinEscape(type || '')}</span><span class="sd-unread">${spinLabel('未读 / 有更新','Unread / updated')}</span><button class="sd-watch" data-action="watch" aria-pressed="${!!pref.watch}">${pref.watch?'★':'☆'} ${spinLabel('自选','Watch')}</button></div>
    <details class="sd-dossier"><summary><div class="sd-identity"><span class="sd-symbol">${spinEscape(e.parentTicker)}</span><span>${spinEscape(e.parentName)}</span><strong>→ ${spinEscape(e.targetName || unknown)} <small>${spinEscape(e.targetTicker)}</small></strong></div>
      <div class="sd-row-meta"><span class="sd-status sd-${spinEscape(e.status)}">${spinStatus(e.status)}</span><time>${spinEscape(e.latestDate || '—')}</time><span class="sd-expand">${spinLabel('查看档案','View dossier')} ⌄</span></div></summary>
      <div class="sd-detail"><div class="sd-facts"><div><span>${spinLabel('母公司','Parent')}</span><b>${spinEscape(e.parentTicker)}</b></div><div><span>${spinLabel('子公司代码','Child ticker')}</span><b>${spinEscape(e.targetTicker || unknown)}</b></div><div><span>${spinLabel('实体对应关系','Entity mapping')}</span><b>${e.identityVerified ? spinLabel('原文规则提取，待人工复核','Extracted from filing; review needed') : unknown}</b></div><div><span>${spinLabel('母公司参考市值','Parent reference cap')}</span><b>${typeof e.parentMarketCap === 'number' ? (lang==='en'?`$${(e.parentMarketCap/10).toFixed(2)}B`:`${e.parentMarketCap.toFixed(1)} 亿美元`) : '—'}</b><small>${spinLabel('缓存快照，非实时','Cached snapshot, not live')}</small></div></div>
...
function spinRenderList(market) {
  ...
  root.querySelector('.sd-results').innerHTML = events.length ? events.map(e=>spinCard(e,prefs[e.id])).join('') : `<div class="sd-empty">${spinLabel('暂无符合条件的事件。可清空搜索或调整筛选。','No matching events. Clear the search or adjust filters.')}</div>`;
...
function spinPaint(market) {
  ...
  const viewNames = {all:spinLabel('全部档案','All dossiers'),watch:spinLabel('我的自选','Watchlist'),review:spinLabel('待核实资料','Needs review'),upcoming:spinLabel('未来 30 天','Next 30 days')};
  const changes = (data.changes || []).filter(c=>spinRecent(c.at,7)).slice(-6).reverse();
  const fieldNames = {status:spinLabel('状态','Status'),targetName:spinLabel('标的','Target'),targetTicker:spinLabel('代码','Ticker'),dates:spinLabel('关键日期','Key dates')};
  root.innerHTML = `<div class="sd-dashboard"><header class="sd-header"><div><p class="sd-eyebrow">${spinLabel('分拆研究工作台','SPIN-OFF RESEARCH')}</p><h2>${market==='hk'?spinLabel('港股分拆','Hong Kong spin-offs'):spinLabel('美股分拆','US spin-offs')}</h2><p class="sd-muted">${spinLabel('先看变化，再核对证据，持续跟踪值得研究的事件。','Track changes, inspect evidence, and follow the events worth researching.')}</p></div><button data-action="refresh">↻ ${spinLabel('刷新数据','Refresh data')}</button></header>
    <p class="sd-updated">${spinLabel('来源数据更新','Source data updated')}: ${spinEscape(data.updatedAt || '—')} · ${spinLabel('不依赖 AI 摘要判断状态','Status does not depend on AI summaries')}</p>
    ...
    <section class="sd-changes"><h3>${spinLabel('最近 7 天的实质变化','Meaningful changes · last 7 days')}</h3>${changes.length ? `<ul>${changes.map(c=>{const e=data.events.find(e=>e.id===c.eventId);return `<li><time>${spinEscape(c.at.slice(0,10))}</time><b>${spinEscape(e?.parentTicker || c.eventId)}</b><span>${c.kind==='new'?spinLabel('新收录','New dossier'):(c.fields || []).map(k=>fieldNames[k] || k).join(' · ')}${c.fields?.includes('status')?` · ${spinStatus(c.fromStatus)} → ${spinStatus(c.toStatus)}`:''}</span></li>`;}).join('')}</ul>` : `<p class="sd-muted">${spinLabel('暂无记录到的实质变化。追踪开始于','No meaningful changes recorded. Tracking began')} ${spinEscape((data.trackingStartedAt || '').slice(0,10))}${spinLabel('；首次整理不会将全部历史事件标为新增。','; initial imports are not counted as new events.')}</p>`}</section>
    <div class="sd-controls"><label class="sd-search">${spinLabel('搜索','Search')}<input type="search" data-filter="query" value="${spinEscape(state.query)}" placeholder="${spinLabel('母公司、子公司或股票代码','Parent, child or ticker')}"></label><label>${spinLabel('状态','Status')}<select data-filter="status"><option value="all">${spinLabel('全部状态','All statuses')}</option>${Object.keys(spinStatusText).map(k=>`<option value="${k}">${spinStatus(k)}</option>`).join('')}</select></label><label>${spinLabel('类型','Type')}<select data-filter="type"><option value="all">${spinLabel('全部（含 REIT）','All (including REITs)')}</option><option value="reit">REIT</option><option value="other">${spinLabel('非 REIT','Non-REIT')}</option></select></label><label>${spinLabel('排序','Sort')}<select data-filter="sort"><option value="latest">${spinLabel('最新公告','Latest filing')}</option><option value="upcoming">${spinLabel('临近日期优先','Upcoming date')}</option><option value="name">${spinLabel('股票代码','Ticker')}</option></select></label></div>
  ...
  root.onclick = event=>{
    ...
    if (button.dataset.action==='watch') {
      ...
      if (spinSave(id,{watch})) { button.setAttribute('aria-pressed',String(watch)); button.textContent=`${watch?'★':'☆'} ${spinLabel('自选','Watch')}`; if (state.view==='watch') spinRenderList(market); else root.querySelector('[data-count="watch"]').textContent=spinBaseEvents(state).filter(e=>spinPrefs()[e.id]?.watch).length; }
      root.querySelector('.sd-storage').hidden=!spinStorageFailed;
    }
  };
