import random
"""Fetch stock prices & estimated buy-in costs for David Tepper (Appaloosa LP) 13F holdings.
Depends only on Python stdlib.

Usage: python3 fetch_prices_tepper.py [--key FINNHUB_KEY]
"""
...
def main():
        with open("tepper.json") as f:
            data = json.load(f)
    except FileNotFoundError:
        print("ERROR: tepper.json not found. Run fetch_13f_tepper.py first.", file=sys.stderr)
    current = data["current"]
    holdings = current["holdings"]
    quarter = current["quarter"]
    prev_quarter = current.get("prevQuarter", quarter)
    print(f"Fetching Prices for Tepper: {len(holdings)} tickers ({quarter} ← {prev_quarter})")
    quotes = {}
    for h in holdings:
        tk = h["ticker"]
    ...
    existing_cb = {}
        with open("prices_tepper.json") as _f:
    ...
    with open("prices_tepper.json", "w") as f:
        json.dump(prices, f, indent=2)
    print(f"\n✅ prices_tepper.json written ({len(quotes)} quotes, {len(cost_basis)} cost basis)")
